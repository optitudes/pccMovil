...
import InteractivePictureCard from './InteractivePictureCard';
...
const LastInteractivePictures = (props) => {
    ...
    const renderItem = ({ item, index }) => {
        return <InteractivePictureCard item={item} horizontal={true} />;
    };
    ...
    useEffect(() => {
        const recentlyPosted = async () => {
            try {
                const res = await httpClient.get("/interactive_picture/"+sourceUrl);
    ...
    >
        ...
        />
            ...
            <TouchableOpacity
                onPress={() => navigation.navigate("searchInteractivePicture")}
                style={{
...
export default LastInteractivePictures
