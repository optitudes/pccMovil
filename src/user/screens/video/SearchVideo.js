...
const SearchVideo = ({ navigation }) => {
    ...
    const [filteredVideosSource, setFilteredVideosSource] = useState([]);
    ...
    const searchFilterFunction = async () => {
        ...
        let word = isNotStringEmpty(searchWord)?searchWord: "a";
            try {
                setIsLoading(true);
                const res = await httpClient.get("/video/search/"+word);
                let videosObtained = res.data.data.data??[];
                var nextPage = res.data.data.next_page_url??"";
                let prevPage = res.data.data.prev_page_url??"";
                setPrevPage(prevPage)
                setNextPage(nextPage)
                setFilteredVideosSource(videosObtained);
            } catch (error) {
    ...
    const renderItem = ({ item }) => {
        return <VideoCard item={item} horizontal={true} style={styles.videoCard} />;
    };
