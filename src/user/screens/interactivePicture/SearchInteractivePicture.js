...
import InteractivePictureCard from '../../../components/InteractivePictureCard';
...
const SearchInteractivePicture = ({ navigation }) => {
    ...
    const searchFilterFunction = async () => {
        ...
        let word = isNotStringEmpty(searchWord)?searchWord: "a";
            try {
                setIsLoading(true);
                const res = await httpClient.get("/interactive_picture/search/"+word);
    ...
    const renderItem = ({ item }) => {
        return <InteractivePictureCard item={item} horizontal={true} style={styles.projectCard} />;
    };
    ...
    return (
        <SafeAreaView style={{ flex: 1 }}>
          ...
          <Text style={styles.title}>Buscar Imagenes interactivas</Text>
            ...
            <TouchableOpacity style={styles.floatingButton} onPress={() => navigation.navigate("interactivePictureForm",{action:"create",pictureId:-1})}>
               <Icon
                name={"plus"}
...
export default SearchInteractivePicture;
